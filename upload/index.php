...
// nginx catch-all server_name of "_" causes the session to be changed every time the page is refreshed when setting the cookie domain to "_" in session_set_cookie_params... I guess PHP doesn't like that value
// HTTP_HOST is better to use for a variety of reasons.  SERVER_NAME should be the fallback.  http://stackoverflow.com/questions/5404811/php-get-domain-name
if(!isset($_SERVER["HTTP_HOST"])){
	$session_cookie_domain_path = $_SERVER['SERVER_NAME'];
}else{
	$session_cookie_domain_path = $_SERVER['HTTP_HOST'];
}
session_set_cookie_params($cookie_lifetime , rtrim(dirname($_SERVER["SCRIPT_NAME"]),"/") . "/", $session_cookie_domain_path, $ssl, true);
session_start();
